...
func SetupDatabase() {
	...
	db.Model(&Shelf{}).Create(&shelf_A)

	shelf_B := Shelf{
		Shelf_name: "Shelf_B",
	}
	db.Model(&Shelf{}).Create(&shelf_B)

	shelf_C := Shelf{
		Shelf_name: "Shelf_C",
	}
	db.Model(&Shelf{}).Create(&shelf_C)
	...
	db.Model(&Payment_Type{}).Create(&cash)

	//payment_type 2
	banking := Payment_Type{
		Payment_Type: "Banking",
	}
	db.Model(&Payment_Type{}).Create(&banking)

	//payment_type 3
	promptPay := Payment_Type{
		Payment_Type: "PromptPay",
	}
	db.Model(&Payment_Type{}).Create(&promptPay)

	//payment_type 4
	credit := Payment_Type{
		Payment_Type: "Debit/Credit Card",
	}
	db.Model(&Payment_Type{}).Create(&credit)
